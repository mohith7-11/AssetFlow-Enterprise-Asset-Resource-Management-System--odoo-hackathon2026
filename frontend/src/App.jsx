...
function App() {
  ...
  const [currentPage, setCurrentPage] = useState('loading'); // 'loading', 'login', 'signup', 'dashboard', 'organization'
  ...
  // On initial mount or token change, fetch current user info
  useEffect(() => {
    if (token) {
  ...
  const loadCategories = () => {
    fetch(`${API_BASE}/api/v1/asset-categories`, {
      headers: { 'Authorization': `Bearer ${token}` }
    })
    .then(res => {
      if (res.ok) return res.json();
  ...
  const onUpdateDepartment = (e) => {
    e.preventDefault();
    fetch(`${API_BASE}/api/v1/departments/${editDept.id}`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
  ...
  const onUpdateCategory = (e) => {
    e.preventDefault();
    fetch(`${API_BASE}/api/v1/asset-categories/${editCat.id}`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
  ...
  const onUpdateUser = (e) => {
    e.preventDefault();
    fetch(`${API_BASE}/api/v1/users/${editUser.id}`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
  ...
  return (
    <div className="app-container">
      ...
      <main className="app-content">
        ...
        <div className="welcome-banner">
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            ...
            {user?.role === 'ADMIN' && (
              <button 
                className={`btn ${currentPage === 'organization' ? 'btn-primary' : 'btn-secondary'}`}
                onClick={() => { setCurrentPage('organization'); setOrgTab('departments'); }}
              >
                Organization Setup
              </button>
            )}
        ...
        {/* PAGE 1: Standard Dashboard (Operational Snapshots) */}
            ...
            <div className="action-box">
              <div>
                ...
                <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>
                  Frequently used shortcuts for resource and lifecycle management (Member 2 & 3 integrations)
                </p>
              </div>
              <div className="action-buttons-list">
                <div className="action-btn-shortcut">
        ...
        {/* PAGE 2: Organization Setup (Admin only) */}
        {currentPage === 'organization' && user?.role === 'ADMIN' && (
          <div className="tabs-container">
            {/* Tabs Header */}
            <div className="tabs-header">
              <button 
                className={`tab-btn ${orgTab === 'departments' ? 'active' : ''}`}
                onClick={() => setOrgTab('departments')}
              >
                🏢 Departments
              </button>
              <button 
                className={`tab-btn ${orgTab === 'categories' ? 'active' : ''}`}
                onClick={() => setOrgTab('categories')}
              >
                🏷️ Asset Categories
              </button>
              <button 
                className={`tab-btn ${orgTab === 'employees' ? 'active' : ''}`}
                onClick={() => setOrgTab('employees')}
              >
                👥 Employee Directory
              </button>
            </div>
            ...
            <div className="tabs-content">
              ...
              {orgTab === 'departments' && (
                <div>
                  ...
                  <form className="grid-form grid-form-2" onSubmit={onCreateDepartment}>
                    <div className="form-group" style={{ margin: 0 }}>
                      ...
                      <input 
                        ...
                        id="dept-name"
                        className="input-control" 
                        placeholder="e.g. Engineering, Facilities, HR" 
                        value={newDeptName}
                  ...
                  {departments.length === 0 ? (
                    <div className="empty-state">No departments registered. Add one above.</div>
                  ) : (
              ...
              {/* TAB 2: Asset Category Management */}
              {orgTab === 'categories' && (
                <div>
                  <div className="section-header">
                  ...
                  {categories.length === 0 ? (
                    <div className="empty-state">No categories registered. Add one above.</div>
                  ) : (
                    <div className="table-wrapper">
                      <table className="custom-table">
                        <thead>
                        ...
                        <tbody>
                          {users.map((emp) => (
                            <tr key={emp.id}>
                              ...
                              <td>
                                <span className="user-badge" style={{ verticalAlign: 'middle' }}>
                                  {emp.role}
                                </span>
                              </td>
                              <td>
              ...
              <div className="modal-body">
                ...
                <div className="form-group">
                  ...
                  <select 
                    id="edit-cat-status"
                    className="select-control"
                    value={editCat.is_active ? 'true' : 'false'}
                    onChange={(e) => setEditCat({ ...editCat, is_active: e.target.value === 'true' })}
                  >
      ...
      <footer style={{
        ...
        © {new Date().getFullYear()} AssetFlow Systems. Monorepo Shared Foundation.
      </footer>
    </div>
  );
}
